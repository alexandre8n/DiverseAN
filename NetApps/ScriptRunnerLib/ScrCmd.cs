    ...
    public class ScrCmd
    {
        ...
        private List<ExprVar> PrepareOperands(ArrayList operands, string oprText)
        {
            ...
            foreach (ExprNode subNode in operands)
            {
                ...
                {
                    ExprVar var = ExecuteOperation(subNode.GetOperationDescriptor(), subNode.GetOperands());
                    vars.Add(var);
                }
                else if (subNode.IsFunctionNode())
                {
                    ExprVar var = ExecuteFunction(subNode.GetFuncName(), subNode.GetOperands());
                    vars.Add(var);
                }
                else if (subNode.IsArray())
                {
                    ExprVar var = CreateArray(subNode);
                    vars.Add(var);
                }
                ...
                {
                    string varName = subNode.GetVarName();
                    ExprVar var = GetVarFromMemory(varName);
        ...
        private ExprVar CreateArray(ExprNode subNode)
        {
            ...
            ExprVarArray result = new ExprVarArray();
            result.CreateArray(vars);
            return result;
        }
