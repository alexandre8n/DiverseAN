...
export default class AuthClient {
  ...
  _abs(path) {
    ...
    // If baseURL is empty or proxy is active, return relative path
    if (!this.baseURL || import.meta.env.DEV) {
      return path.startsWith("/") ? path : `/${path}`;
    }
    // Otherwise use full baseURL
  ...
  async _rawFetch(inputA, init) {
    const input = this._abs(inputA);
    const opts = {
      ...init,
      credentials: "include",
    };

    if (!this.timeoutMs) return fetch(input, opts);
  ...
  async _rawFetch_old(inputA, init) {
